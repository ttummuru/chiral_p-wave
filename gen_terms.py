...
def rotate(lattice, theta):
    ...
    '''
    rot = np.matrix([[np.cos(theta), -1.0 * np.sin(theta)],
...
def generate(a, d, vec, interlayerNNnumber, interlayerNNdistance, offsetamount=0.0, plothoppingmaps=True, rho=0.39, nx=1, ny=1):
    ...
    # plots the hopping map as they are generated. (random numbering)
    plothops = False
    ...
    # orange to the right
    layer2 = rotate(lat, -1.0 * np.arctan(vec[0] / vec[1]))
    # translate layer 2
    offsetangle = np.pi / 4 - np.arctan(vec[0] / vec[1])
    layer2 = translate(layer2, offsetamount * a * 0.5 * np.sqrt(2) * np.cos(
        offsetangle), offsetamount * a * 0.5 * np.sqrt(2) * np.sin(offsetangle))
    ...
    '''
    def inunit(x, y):
        if 0 <= x < uc_size_x and 0 <= y < uc_size_y:
            return True
        else:
            return False
    ...
    def plotbase(fig):
        ...
        rect = plt.Rectangle((0, 0), uc_size_x, uc_size_y,
                             color='k', alpha=0.1)
        ax.add_patch(rect)
    ...
    def get_hoppings(layer, neighbor_range=1):
        ...
        if layer == 1 or layer == 2:
            ...
            for hop in layer_hoppings:
                ...
                if dep_in:
                    ...
                    reciprocal = [int(desx // (uc_size_x)),
                                  int(desy // (uc_size_y))]
                    if plothops:
                        plt.text(depx + 0.1 * a, depy + 0.1 * a,
                                 hop[0])  # plot the old labeling
                        if des_in:
                            plt.text(desx + 0.1 * a, desy + 0.1 * a,
                                     hop[1])  # plot the old labeling

                        plt.arrow(depx, depy, dx, dy, width=0.02,
                                  length_includes_head=True, alpha=0.5)
                        plt.text(depx + 0.5 * dx, depy + 0.5 *
                                 dy, bdirection, color='blue')

                    hops.append([hop[0], hop[1], bdirection,
                                 reciprocal, [layer, layer]])
        ...
        if layer == 12:
            ...
            for siteindex in range(len(largeruc1)):
                hopfrom = largeruc1[siteindex]
                NNBS = findNNBS(
                    siteindex, closest_n=interlayerNNnumber, maxdistance=interlayerNNdistance)
                for nneighbor in NNBS:
                    ...
                    if inunit(depx, depy):
                        palpha = 0.4
                        if plothops:
                            plt.text(depx + 0.05 * a, depy + 0.05 * a,
                                     siteindex)  # plot the old labeling
                            plt.text(desx + 0.05 * a, desy - 0.05 * a,
                                     targetsite)  # plot the old labeling
                        hops.append(
                            [siteindex, targetsite, couplingfactor, reciprocal, [1, 2]])
                        #print ("l1 site "+str(siteindex)+" to l2 site "+str(targetsite), reciprocal)
                        if plothops:
                            plt.arrow(depx, depy, dx, dy, width=0.02 * couplingfactor,
                                      length_includes_head=True, alpha=palpha)
        ...
        for hop in hops:
            ...
            if rec != [0, 0]:
                ...
                # get the coordinates of this point and subtract the reciprocal vector
                intheBZ = largeruc[des] - subtract
    ...
    def plothopmap(hopstoplot, plottitle):
        ...
        for hop in hopstoplot:
            ...
            if hop[4][0] != hop[4][1]:
                ...
                if des_in:
                    plt.text(desx + 0.05 * a, desy - 0.05 *
                             a, hop[1], color="black")
            if dep_in:
                plt.text(depx + 0.05 * a, depy + 0.05 *
                         a, hop[0], color=textcolor)

                if not (dx == 0.0 and dy == 0.0):
                    plt.arrow(depx, depy, dx, dy, width=normalwidth,
                              length_includes_head=True, alpha=0.2)
    ...
    for hop in hops2:
        hop[0] -= singlelayerbasissize
        hop[1] -= singlelayerbasissize
    ...
    # all = {'1': hops1, '2': hops2, '11': hops11, '22': hops22, '12': hops12}

    all = {'1': np.array(hops1), '2': np.array(hops2),
           # '11': np.array(hops11), '22': np.array(hops22),
           '12': np.array(hops12)}
